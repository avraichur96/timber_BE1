...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--username", type=str, help="Username for the new user"
        )
        parser.add_argument("--email", type=str, help="Email for the new user")
        parser.add_argument(
            "--password", type=str, help="Password for the new user"
        )
        parser.add_argument(
            "--first-name", type=str, help="First name for the new user"
        )
        parser.add_argument(
            "--last-name", type=str, help="Last name for the new user"
        )
        parser.add_argument(
            "--superuser", action="store_true", help="Create as superuser"
        )
        parser.add_argument(
            "--staff", action="store_true", help="Create as staff user"
        )
    ...
    def handle(self, *args, **options):
        ...
        if not username or not email:
            self.stdout.write(
                self.style.ERROR("Username and email are required")
            )
            return
        ...
        # Get optional fields
        first_name = (
            options.get("first_name") or input("First name (optional): ") or ""
        )
        last_name = (
            options.get("last_name") or input("Last name (optional): ") or ""
        )
